 ...
 * 部位与症状关系抓取
 */
public class JBMediaBodyCrawler {
    ...
    private static Map<String, List<Symptom>> bodyDisease = new LinkedHashMap<>();
    private static final String urlPrefix = "http://jib.xywy.com";

    public static void main(String[] args) {
        try {
            JBMediaBodyCrawler crawler = new JBMediaBodyCrawler();
            crawler.getDepartment("http://jib.xywy.com/html/toubu.html");
            ...
            bodyDisease.forEach((k, v) -> {
                System.out.println(k + ":" + v);
            });

            crawler.createExcel(bodyDisease);
    ...
    private void createExcel(Map<String, List<Symptom>> bodyZhengzhuang) {
        ...
        bodyZhengzhuang.forEach((k, v) -> {
            ...
            v.forEach(e -> {
                ...
                row.put("疾病", e.getTitle());
                row.put("疾病链接", e.getHref());
                rows.add(row);
            });
        });
...
//        ArrayList<Map<String, Object>> rows = CollUtil.newArrayList(row1, row2);
        // 通过工具类创建writer
        ExcelWriter writer = ExcelUtil.getWriter("G:/buweiAndJB.xlsx");
        // 合并单元格后的标题行，使用默认标题样式
